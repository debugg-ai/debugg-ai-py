...
from debugg_ai_sdk.integrations.spark.spark_driver import (
    _set_app_properties,
    _start_debugg_ai_listener,
    DebuggAIListener,
    SparkIntegration,
)
from debugg_ai_sdk.integrations.spark.spark_worker import SparkWorkerIntegration
...
def debugg_ai_init_with_reset(debugg_ai_init):
    from debugg_ai_sdk.integrations import _processed_integrations

    yield lambda: debugg_ai_init(integrations=[SparkIntegration()])
...
def test_set_app_properties(create_spark_context):
    ...
    assert spark_context.getLocalProperty("debugg_ai_app_name") == "Testing123"
    # applicationId generated by sparkContext init
    assert (
        spark_context.getLocalProperty("debugg_ai_application_id")
        == spark_context.applicationId
    )


def test_start_debugg_ai_listener(create_spark_context):
    spark_context = create_spark_context()
    gateway = spark_context._gateway
    assert gateway._callback_server is None

    _start_debugg_ai_listener(spark_context)
...
def test_debugg_ai_listener_on_job_start(debugg_ai_listener):
    listener = debugg_ai_listener
...
def test_debugg_ai_listener_on_job_end(debugg_ai_listener, job_result, level):
    listener = debugg_ai_listener
    with patch.object(listener, "_add_breadcrumb") as mock_add_breadcrumb:
...
def test_debugg_ai_listener_on_stage_submitted(debugg_ai_listener):
    listener = debugg_ai_listener
    with patch.object(listener, "_add_breadcrumb") as mock_add_breadcrumb:
...
def test_debugg_ai_listener_on_stage_submitted_no_attempt_id(debugg_ai_listener):
    listener = debugg_ai_listener
    with patch.object(listener, "_add_breadcrumb") as mock_add_breadcrumb:
...
def test_debugg_ai_listener_on_stage_submitted_no_attempt_id_or_number(debugg_ai_listener):
    listener = debugg_ai_listener
    with patch.object(listener, "_add_breadcrumb") as mock_add_breadcrumb:
...
def test_debugg_ai_listener_on_stage_completed_success(
    debugg_ai_listener, get_mock_stage_completed
):
    listener = debugg_ai_listener
    with patch.object(listener, "_add_breadcrumb") as mock_add_breadcrumb:
...
def test_debugg_ai_listener_on_stage_completed_failure(
    debugg_ai_listener, get_mock_stage_completed
):
    listener = debugg_ai_listener
    with patch.object(listener, "_add_breadcrumb") as mock_add_breadcrumb:
...
def test_spark_worker(monkeypatch, debugg_ai_init, capture_events, capture_exceptions):
    ...
    debugg_ai_init(integrations=[SparkWorkerIntegration()])
