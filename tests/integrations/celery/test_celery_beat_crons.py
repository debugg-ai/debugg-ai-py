...
from debugg_ai_sdk.crons import MonitorStatus
...
from debugg_ai_sdk.integrations.celery.utils import _get_humanized_interval
...
def test_crons_task_success():
    ...
    with mock.patch(
        "debugg_ai_sdk.integrations.celery.beat.capture_checkin"
    ) as mock_capture_checkin:
        with mock.patch(
            "debugg_ai_sdk.integrations.celery.beat._now_seconds_since_epoch",
            return_value=500.5,
        ):
...
def test_exclude_beat_tasks_option(
    ...
    with mock.patch(
        "debugg_ai_sdk.integrations.celery.beat.Scheduler", fake_scheduler
    ) as Scheduler:  # noqa: N806
        with mock.patch(
            "debugg_ai_sdk.integrations.celery.debugg_ai_sdk.get_client",
            return_value=fake_client,
        ):
            with mock.patch(
                "debugg_ai_sdk.integrations.celery.beat._get_monitor_config",
...
def test_exclude_redbeat_tasks_option(
    ...
    with mock.patch(
        "debugg_ai_sdk.integrations.celery.beat.RedBeatScheduler", fake_redbeat_scheduler
