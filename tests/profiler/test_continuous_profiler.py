...
def test_continuous_profiler_auto_start_and_manual_stop(
    ...
    with debugg_ai_sdk.start_transaction(name="profiling"):
        with debugg_ai_sdk.start_span(op="op"):
            time.sleep(0.05)
...
def test_continuous_profiler_auto_start_and_stop_sampled(
    ...
    for _ in range(3):
        ...
        with debugg_ai_sdk.start_transaction(name="profiling 1"):
            assert get_profiler_id() is not None, "profiler should be running"
            with debugg_ai_sdk.start_span(op="op"):
                time.sleep(0.1)
            assert get_profiler_id() is not None, "profiler should be running"
        ...
        with debugg_ai_sdk.start_transaction(name="profiling 2"):
            assert get_profiler_id() is not None, "profiler should be running"
            with debugg_ai_sdk.start_span(op="op"):
                time.sleep(0.1)
            assert get_profiler_id() is not None, "profiler should be running"
