#!/usr/bin/env bash
#
# Builds and deploys the `DebuggAIPythonServerlessSDK-local-dev` AWS Lambda layer (containing the DebuggAI SDK)
#
...
aws lambda publish-layer-version \
    --layer-name "DebuggAIPythonServerlessSDK-local-dev" \
    --region "eu-central-1" \
    --zip-file "fileb://dist/$ZIP" \
    --description "Local test build of DebuggAIPythonServerlessSDK (can be deleted)" \
    --compatible-runtimes python3.7 python3.8 python3.9 python3.10 python3.11 \
    --no-cli-pager

echo "Done deploying zipped Lambda layer to AWS as 'DebuggAIPythonServerlessSDK-local-dev'."
